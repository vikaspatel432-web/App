...
import { addProjectAccess } from "@/app/admin/actions";

export function NewProjectAccessForm({ clientOrgId }: { clientOrgId: string }) {
  const [state, action, pending] = useActionState(addProjectAccess, undefined);
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-[200px]">
        <label className="mb-1 block text-xs text-neutral-400">Speckle project ID</label>
        <input
      ...
      <div className="flex-1 min-w-[160px]">
        <label className="mb-1 block text-xs text-neutral-400">Label (optional)</label>
        <input
          name="label"
          className="w-full rounded-md border border-neutral-700 bg-neutral-950 px-3 py-2 text-sm text-white outline-none focus:border-blue-500"
        />
